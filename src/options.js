...
$(document).ready(function() {
    ...
    let c_options = JSON.parse(bkg_page.getOption('displayOption'));
    ...
    let w_options = JSON.parse(bkg_page.getOption('watchList'));
    ...
    let wn_options = JSON.parse(bkg_page.getOption('watchNotifyList'));
    ...
    $('.chat-display .display-option .option .button').click(function() {
        ...
        $(this).addClass('on');
        let displayOption = JSON.parse(bkg_page.getOption('displayOption'));
        let type = $(this).attr('option');
        if (type === 'on') {
            if (displayOption.indexOf(classes) === -1) {
                displayOption.push(classes);
            }
        } else {
            let index = displayOption.indexOf(classes);
            if (index !== -1) {
    ...
    $('.watcher-options .watcher-option .option .button').click(function() {
        ...
        $(this).addClass('on');
        let watchList = JSON.parse(bkg_page.getOption('watchList'));
        let type = $(this).attr('option');
        if (type === 'on') {
            if (watchList.indexOf(classes) === -1) {
                watchList.push(classes);
            }
        } else {
            let index = watchList.indexOf(classes);
            if (watchList.indexOf(classes) !== -1) {
    ...
    $('.watcher-notify-options .watcher-notify-option .option .button').click(function() {
        ...
        $(this).addClass('on');
        let watchNotifyList = JSON.parse(bkg_page.getOption('watchNotifyList'));
        let type = $(this).attr('option');
        if (type === 'on') {
            if (watchNotifyList.indexOf(classes) === -1) {
                watchNotifyList.push(classes);
            }
        } else {
            let index = watchNotifyList.indexOf(classes);
            if (index !== -1) {
                watchNotifyList.splice(index, 1);
            }
        }
        bkg_page.setOption('watchNotifyList', JSON.stringify(watchNotifyList));
    });
    function initUpList() {
        let list = Live.get('favouritesList');
        let idList = Live.get('favouritesIdList');
        let ListDom = $('#up-list');
        each(list, function(i) {
            let upInfo = list[i];
            let del = $('<button class="button on delete" />').text('取消关注');
            let l = $('<li />').attr('roomId', upInfo.roomId).append($('<span class="upName"/>').text(upInfo.upName), del);
            ListDom.append(l);
            del.click(function() {
                let index = idList.indexOf(parseInt(i));
                if (index !== -1) {
    ...
    let updateInfo = JSON.parse(bkg_page.getOption('crx_update'));

    if (updateInfo.version && bkg_page.compareVersion(updateInfo.version, chrome.runtime.getManifest().version) > 0) {
        $('#update').show();
    ...
    case 'update':
        swal({
            title: '升级成功',
            text: '您已成功升级至哔哩哔哩助手版本 v' + chrome.runtime.getManifest().version + '！请参阅右侧有关扩展更新内容，再次感谢您对哔哩哔哩助手项目的支持。',
            type: 'success',
    ...
    case 'install':
        swal({
            title: '安装成功',
            text: '感谢您安装哔哩哔哩助手版本 v' + chrome.runtime.getManifest().version + '！请根据您的需要在左侧更改扩展的选项，右侧为有关扩展的相关介绍和说明。使用此扩展前请您阅读相关<a href="http://addons-privacy.com/" target="_blank">使用协议和隐私策略</a>。',
            type: 'success',
    ...
    case 'new':
        if (updateInfo.version) {
            swal({
                title: '发现新版本',
                text: '发现新版哔哩哔哩助手: v' + updateInfo.version + '<br/>您当前使用的版本是: v' + chrome.runtime.getManifest().version + '<br/>如果您不能通过 Google 自动更新扩展或者在使用上遇到严重的问题，建议您参阅右侧信息，手动更新。',
                type: 'warning',
                confirmButtonText: '好的',
